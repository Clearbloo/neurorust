...
impl DenseLayer {
    ...
    pub fn grad_layer(
        ...
        let output_gradient = self.activation.calculate_gradient(output_gradient);
...
mod test_layer {
    ...
    fn test_grad_layer() {
        ...
        let (weight_gradient, bias_gradient, _input_gradient) = layer.grad_layer(
            &input, &output, // &target
        );

        let _expected_input_gradient = arr2(&[[1.0, 0.1], [1.0, 1.0]]);
        let expected_weight_gradient = arr2(&[[3.0, 1.2], [2.0, -0.7]]); // FIXME - Update when implemented
        let expected_bias_gradient = arr2(&[[2.0, 1.1]]); // FIXME - Update when implemented
